...
class TestTimer(unittest.IsolatedAsyncioTestCase):
    async def test_timed_debug_on(self):
        ...
        with patch("migration_engine.utils.timer.logger") as mock_logging:
            ...
            mock_logging.debug.assert_called_once()
            self.assertIn("[TIMING]", mock_logging.debug.call_args[0][0])
            self.assertIn(label, mock_logging.debug.call_args[0])
            self.assertIn(
                mock_page.Id.Id, mock_logging.debug.call_args[0]
            )
    ...
    async def test_timed_debug_off(self):
        ...
        with patch("migration_engine.utils.timer.logging") as mock_logging:
            mock_logger = mock_logging.return_value = MagicMock()
            from migration_engine.utils.timer import timed
            # Create a mock page inside the test to avoid issues with global scope
            mock_page = create_mock_page(
                UUID('{12345678-1234-5678-1234-567812345678}'),
